...
'
' start_com_port(1,SCANNER)
'
' Public Const SCANNER As String = "Baud=1200 Data=7 Parity=N"
' Public Const VOLTMETER As String = "Baud=9600 Data=8 Parity=N"
'
'------------------------------------------------------------------------------------------
...
Private Declare PtrSafe Function Set_Port_Lines Lib "Kernel32.dll" Alias "EscapeCommFunction" (ByVal Port_Handle As LongPtr, ByVal Signal_Function As Long) As Boolean
...
Result_Text = TEXT_SUCCESS
Timer_Text = TEXT_READ_EQUALS & READ_TIMEOUT & TEXT_MS & TEXT_WRITE_EQUALS & WRITE_CONSTANT & TEXT_MS
...
If Port_Valid Then
    ...
    If Port_Started(Port_Number) Then
        ...
        If Signal_Valid Then
            ...
            Signal_Result = Set_Port_Lines(COM_PORT(Port_Number).Handle, Signal_Function)
            COM_PORT(Port_Number).DLL_Error = Err.LastDllError
            ...
            Else
                ...
                Result_Text = TEXT_FAILURE
                Detail_Text = Signal_Text_4 & DLL_ERROR_TEXT(Err.LastDllError)
...
Dim Signal_Result As Boolean
Dim RTS_Text As String, Result_Text As String, Detail_Text As String, Debug_Text As String

Const SIGNAL_RTS_ON As Long = LONG_3
Const SIGNAL_RTS_OFF As Long = LONG_4

Const TEXT_RTS_ON As String = " State = On "
Const TEXT_RTS_OFF As String = " State = Off "

Const Signal_Text_1 As String = "SIGNALLING"
Const Signal_Text_2 As String = " Setting Port RTS Signal "
Const Signal_Text_3 As String = " Signal Com Port Success, Result = "
Const Signal_Text_4 As String = " Signal Com Port Failed, "

RTS_Text = IIf(RTS_State, TEXT_RTS_ON, TEXT_RTS_OFF)
RTS_Signal = IIf(RTS_State, SIGNAL_RTS_ON, SIGNAL_RTS_OFF)
...
If Port_Valid Then
    ...
    If Port_Started(Port_Number) Then
            ...
            If Port_Debug Then
                Debug_Text = COM_PORT(Port_Number).Name & Signal_Text_2 & RTS_Signal & TEXT_COMMA & RTS_Text
                PRINT_DEBUG_TEXT Module_Name, Signal_Text_1, Debug_Text
            End If
            
            Signal_Result = Set_Port_Lines(COM_PORT(Port_Number).Handle, RTS_Signal)
            COM_PORT(Port_Number).DLL_Error = Err.LastDllError
                
            If Signal_Result Then
            
                Result_Text = TEXT_SUCCESS
                Detail_Text = Signal_Text_3 & Signal_Result
