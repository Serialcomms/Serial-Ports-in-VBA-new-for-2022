...
Private Declare PtrSafe Function Query_Com_DCB Lib "Kernel32.dll" Alias "GetCommState" (ByVal Port_Handle As LongPtr, ByRef Port_DCB As DEVICE_CONTROL_BLOCK) As Boolean
Private Declare PtrSafe Function Apply_Com_DCB Lib "Kernel32.dll" Alias "SetCommState" (ByVal Port_Handle As LongPtr, ByRef Port_DCB As DEVICE_CONTROL_BLOCK) As Boolean
...
Temp_Handle = Com_Port_Open(Device_Path, GENERIC_RW, OPEN_EXCLUSIVE, vbNullString, OPEN_EXISTING, CREATE_FILE_FLAGS)
...
If GET_PORT_CONFIG(Port_Number) Then          ' get existing com port config (baud, parity etc.) into device control block
    ...
    If New_Settings Then
        ...
        If Temp_Build Then
            ...
            Temp_Result = Apply_Com_DCB(COM_PORT(Port_Number).Handle, COM_PORT(Port_Number).DCB)
            COM_PORT(Port_Number).DLL_Error = Err.LastDllError
            Error_Text = Decode_System_Errors(COM_PORT(Port_Number).DLL_Error)
            
            Success_Text = Set_Text_4 & GET_PORT_SETTINGS(Port_Number)
            Failure_Text = Set_Text_7 & Error_Text
...
Temp_Result = Query_Com_DCB(COM_PORT(Port_Number).Handle, COM_PORT(Port_Number).DCB)
COM_PORT(Port_Number).DLL_Error = Err.LastDllError
...
If Port_Valid Then
    ...
    If Port_Started(Port_Number) Then
        ...
        If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, TEXT_RESULT, COM_PORT(Port_Number).Name & Temp_Text & Temp_Result)
...
If Port_Valid Then
    ...
    If Port_Started(Port_Number) Then
        ...
        Temp_Result = Synchronous_Read(COM_PORT(Port_Number).Handle, Get_Character, LONG_1, Read_Byte_Count)
        COM_PORT(Port_Number).DLL_Error = Err.LastDllError: Error_Text = Decode_System_Errors(COM_PORT(Port_Number).DLL_Error)
...
If Port_Debug Then
    ...
    Failure_Text = Clear_Text_2
    Success_Text = Clear_Text_3
    Result_Text = IIf(Temp_Result, TEXT_SUCCESS, TEXT_FAILURE)
    Detail_Text = IIf(Temp_Result, Success_Text, Failure_Text)
    
    Call PRINT_SHOW_TEXT(Module_Name, Result_Text, COM_PORT(Port_Number).Name & Detail_Text, TEXT_EQUALS_SPACE & Error_Text)
...
Dim TIMESTAMP_TIME As SYSTEMTIME, TIMESTAMP_STRING As String * LONG_14
