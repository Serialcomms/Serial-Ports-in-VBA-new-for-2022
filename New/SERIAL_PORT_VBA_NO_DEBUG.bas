...
.Settings = GET_PORT_SETTINGS(Port_Number)
...
Clear_Result = Com_Port_Clear(.Handle, .Port_Errors, .Status)
        ...
        Do
            ...
            If Temp_Result And Not .Buffers.Read_Buffer_Empty Then
                ...
                Select Case .Buffers.Synchronous_Bytes_Read
                    ...
                    Case Is = .Timers.Timeslice_Bytes
...
.Timeouts.Read_Interval_Timeout = READ_TIMEOUT            ' Timeouts not used for file reads.
...
If Port_Ready(Port_Number) Then
        ...
        If Com_Port_Clear(.Handle, .Port_Errors, .Status) Then Temp_Queue = .Status.QUEUE_IN
...
If Port_Ready(Port_Number) Then
    ...
    Temp_Result = Get_Port_Modem(.Handle, .Port_Signals)
    
    If Temp_Result Then Signal_State = IIf(.Port_Signals And CTS_ON, True, False)
...
If Port_Ready(Port_Number) Then
    ...
    If Temp_Result Then Signal_State = IIf(.Port_Signals And DSR_ON, True, False)
...
If Port_Ready(Port_Number) Then
    ...
    If Temp_Result Then Signal_State = IIf(.Port_Signals And RING_ON, True, False)
