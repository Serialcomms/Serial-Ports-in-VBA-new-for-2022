...
Detail_Text = Start_Text_2 & Port_Number & Start_Text_3 & COM_PORT(Port_Number).Handle
...
Else
Detail_Text = Start_Text_5 & Port_Number
End If

Else
Detail_Text = Start_Text_6 & COM_PORT(Port_Number).Handle
End If
...
Const Config_Text_6 As String = " Failed to Set Port Configuration "

Temp_Result = False

Temp_Settings = CLEAN_PORT_SETTINGS(Port_Settings)

Selected_Settings = IIf(Len(Temp_Settings) > LONG_0, Config_Text_1, Config_Text_2)

If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, TEXT_STARTUP, COM_PORT(Port_Number).Name & Selected_Settings)

If SET_PORT_CONFIG(Port_Number, Temp_Settings) Then
If SET_PORT_TIMERS(Port_Number) Then
If SET_PORT_VALUES(Port_Number) Then
      Temp_Result = True
      Result_Text = TEXT_SUCCESS: Detail_Text = Config_Text_3 & GET_PORT_SETTINGS(Port_Number)
...
Else: Result_Text = TEXT_FAILURE: Detail_Text = Config_Text_6 & Port_Settings: End If
...
Const Set_Text_6 As String = " Failed to build DCB, Last Error = "
Const Set_Text_7 As String = " Failed to apply configuration settings, Last Error "
...
If GET_PORT_CONFIG(Port_Number) Then          ' get existing com port config (baud, parity etc.) into device control block
    ...
    If New_Settings Then
        ...
        If Temp_Build Then
            ...
            Temp_Result = Set_Com_State(COM_PORT(Port_Number).Handle, COM_PORT(Port_Number).DCB)
            COM_PORT(Port_Number).DLL_Error = Err.LastDllError
            
            Success_Text = Set_Text_4 & GET_PORT_SETTINGS(Port_Number)
            Failure_Text = Set_Text_7 & Decode_System_Errors(COM_PORT(Port_Number).DLL_Error)
        ...
        Else
            ...
            Temp_Result = False
            Result_Text = TEXT_FAILURE
            Detail_Text = Set_Text_6 & Decode_System_Errors(COM_PORT(Port_Number).DLL_Error)
...
Dim Effective_Byte_Count As Long, Read_Buffer_Length As Long, Bytes_Per_Second As Long

Const READ_BYTES As String = "READ_BYTES"
Const READ_BUFFER As String = "READ_BUFFER"
...
Bytes_Per_Second = COM_PORT(Port_Number).Timers.Bytes_Per_Second
...
If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, READ_BUFFER, COM_PORT(Port_Number).Name & Values_Text_2 & Read_Buffer_Length)
If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, READ_BYTES, COM_PORT(Port_Number).Name & Values_Text_3 & Effective_Byte_Count)

Else   ' read buffer size not > 0

Temp_Result = False

If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, READ_BUFFER, COM_PORT(Port_Number).Name & Values_Text_1 & Read_Buffer_Length)
...
Const Stop_Text_6 As String = " with Handle "
...
If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, TEXT_STOPPING, Temp_Name & Stop_Text_1 & Port_Number & Stop_Text_6 & Temp_Handle)
...
Const TEXT_WAIT_START As String = "WAIT_START"
Const TEXT_WAIT_RESULT As String = "WAIT_RESULT"
...
If Port_Valid Then
    ...
    If Port_Started(Port_Number) Then
        ...
        If Port_Debug Then
            ...
            Call PRINT_DEBUG_TEXT(Module_Name, TEXT_WAIT_START, COM_PORT(Port_Number).Name & Wait_String)
            ...
            Call PRINT_DEBUG_TEXT(Module_Name, TEXT_WAIT_RESULT, COM_PORT(Port_Number).Name & Wait_String)
...
If Port_Debug Then Port_Name = COM_PORT(Port_Number).Name
...
If Clear_Result Then
    ...
    If Not Data_Waiting Then
        ...
        If Not Wait_Expired Then
            ...
            If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, TEXT_WAITING, Port_Name & Debug_Text)
...
Dim Temp_Result As Boolean
Dim Progress_Text As String, Success_Text As String, Failure_Text As String

Const Temp_Text_1 As String = " Port not started, no data available, Last DLL Error "
Const Temp_Text_2 As String = " Existing Port Settings, "
...
Failure_Text = Temp_Text_1 & Decode_System_Errors(COM_PORT(Port_Number).DLL_Error)
Success_Text = Temp_Text_2 & GET_PORT_SETTINGS_FROM_DCB(Port_Number)
Progress_Text = IIf(Temp_Result, Success_Text, Failure_Text)

Call PRINT_DEBUG_TEXT(Module_Name, TEXT_CONFIG, COM_PORT(Port_Number).Name & Progress_Text)
...
Const Temp_Text_2 As String = " Synchronous Read, Last Error "
...
Else
    ...
    Error_Text = Decode_System_Errors(COM_PORT(Port_Number).DLL_Error)
    Debug_Text = COM_PORT(Port_Number).DLL_Error & TEXT_EQUALS & Error_Text
    If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, TEXT_SYNC_READ, COM_PORT(Port_Number).Name & Temp_Text_2 & Debug_Text)
...
Write_Complete = True
If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, TEXT_SYNC_WRITE, COM_PORT(Port_Number).Name & Temp_Text_2 & Write_Buffer_Length)
If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, TEXT_SYNC_WRITE, COM_PORT(Port_Number).Name & Temp_Text_3 & COM_PORT(Port_Number).Buffers.Synchronous_Bytes_Sent)

Else

Write_Complete = False
Error_Text = Decode_System_Errors(COM_PORT(Port_Number).DLL_Error)
If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, TEXT_SYNC_WRITE, COM_PORT(Port_Number).Name & Temp_Text_1 & COM_PORT(Port_Number).DLL_Error & " = " & Error_Text)
...
Dim Transmit_Result As Boolean
...
If Port_Valid Then
    ...
    If Port_Started(Port_Number) Then
        ...
        Transmit_Result = TRANSMIT_COM_PORT(Port_Number, CStr(Send_Variable))
        If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, TEXT_RESULT, COM_PORT(Port_Number).Name & Temp_Text_1 & Transmit_Result)
...
If Port_Valid Then
        ...
        Write_Result = Synchronous_Write(COM_PORT(Port_Number).Handle, Put_Character, LONG_1, Write_Byte_Count, ByVal vbNullString)
        COM_PORT(Port_Number).DLL_Error = Err.LastDllError: Error_Text = Decode_System_Errors(COM_PORT(Port_Number).DLL_Error)
...
Dim Error_Text As String, Get_Character As String * LONG_1               ' must be fixed length 1

If Port_Valid Then

        If Port_Started(Port_Number) Then

        Temp_Result = Synchronous_Read(COM_PORT(Port_Number).Handle, Get_Character, LONG_1, Read_Byte_Count, ByVal vbNullString)
        COM_PORT(Port_Number).DLL_Error = Err.LastDllError: Error_Text = Decode_System_Errors(COM_PORT(Port_Number).DLL_Error)
...
If Port_Debug Then
    Error_Text = Decode_System_Errors(COM_PORT(Port_Number).DLL_Error)
    Temp_String = COM_PORT(Port_Number).Name & Purge_Text_1 & Temp_Result & TEXT_COMMA & Purge_Text_2 & Error_Text
    Call PRINT_DEBUG_TEXT(Module_Name, "PURGE", Temp_String)
End If
...
Const TEXT_WRITE_EQUALS As String = ", Write = "

Const Set_Text_1 As String = " Port Timers Not Set, Last Error = "
Const Set_Text_2 As String = " Port Read/Write Timers Applied,"
...
If Port_Debug Then
    ...
    Error_Text = Decode_System_Errors(COM_PORT(Port_Number).DLL_Error)
    Timer_Text = TEXT_READ_EQUALS & READ_TIMEOUT & TEXT_MS & TEXT_WRITE_EQUALS
...
Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_2, Port_Name & " Read Interval     ", TEXT_EQUALS_SPACE & Temp_Timer(LONG_1))
Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_2, Port_Name & " Read Constant     ", TEXT_EQUALS_SPACE & Temp_Timer(LONG_2))
Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_2, Port_Name & " Read Multiplier   ", TEXT_EQUALS_SPACE & Temp_Timer(LONG_3))
Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_3, Port_Name & " Write Constant    ", TEXT_EQUALS_SPACE & Temp_Timer(LONG_4))
Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_3, Port_Name & " Write Multiplier  ", TEXT_EQUALS_SPACE & Temp_Timer(LONG_5))

Else

Error_Text = Decode_System_Errors(COM_PORT(Port_Number).DLL_Error)
Call PRINT_DEBUG_TEXT(Module_Name, TEXT_ERROR, Temp_Text_4 & Port_Name & Temp_Text_5 & Error_Text)
...
Call PRINT_SHOW_TEXT(TEXT_PORT_ERRORS, "OVERFLOW ", Port_Name & Error_Text_1, TEXT_EQUALS_SPACE & Temp_Error(LONG_1))
Call PRINT_SHOW_TEXT(TEXT_PORT_ERRORS, "OVERRUN  ", Port_Name & Error_Text_2, TEXT_EQUALS_SPACE & Temp_Error(LONG_2))
Call PRINT_SHOW_TEXT(TEXT_PORT_ERRORS, "PARITY   ", Port_Name & Error_Text_3, TEXT_EQUALS_SPACE & Temp_Error(LONG_3))
Call PRINT_SHOW_TEXT(TEXT_PORT_ERRORS, "FRAMING  ", Port_Name & Error_Text_4, TEXT_EQUALS_SPACE & Temp_Error(LONG_4))
Call PRINT_SHOW_TEXT(TEXT_PORT_ERRORS, "BREAK    ", Port_Name & Error_Text_5, TEXT_EQUALS_SPACE & Temp_Error(LONG_5))
...
Const Modem_Text_3 As String = "Error retrieving Modem Status, Last Error = "
Const Modem_Text_4 As String = " Clear to Send (from device)    CTS  "
Const Modem_Text_5 As String = " Data Set (Modem/Device) Ready  DSR  "
Const Modem_Text_6 As String = " Ring Signal (from modem)       RING "
Const Modem_Text_7 As String = " Receive Line Signal Detect     RLSD "
...
Call PRINT_SHOW_TEXT(Modem_Text_1, Modem_Text_2, COM_PORT(Port_Number).Name & Modem_Text_5, TEXT_EQUALS_SPACE & SIGNAL_DSR)
'Call PRINT_SHOW_TEXT(Modem_Text_1, Modem_Text_2, COM_PORT(Port_Number).Name & Modem_Text_6, TEXT_EQUALS_SPACE & SIGNAL_RNG)
'Call PRINT_SHOW_TEXT(Modem_Text_1, Modem_Text_2, COM_PORT(Port_Number).Name & Modem_Text_7, TEXT_EQUALS_SPACE & SIGNAL_RLS)

Else

Error_Text = Decode_System_Errors(COM_PORT(Port_Number).DLL_Error)

Call PRINT_DEBUG_TEXT(Module_Name, TEXT_ERROR, COM_PORT(Port_Number).Name & Modem_Text_3 & Error_Text)
...
Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_2, Port_Name & " Transmission Waiting for CTS     ", TEXT_EQUALS_SPACE & Temp_Wait(LONG_1))
Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_2, Port_Name & " Transmission Waiting for DSR     ", TEXT_EQUALS_SPACE & Temp_Wait(LONG_2))
Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_2, Port_Name & " Transmission Waiting for RLSD    ", TEXT_EQUALS_SPACE & Temp_Wait(LONG_3))
Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_2, Port_Name & " Transmission Waiting (XOFF Hold) ", TEXT_EQUALS_SPACE & Temp_Wait(LONG_4))
Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_2, Port_Name & " Transmission Waiting (XOFF Sent) ", TEXT_EQUALS_SPACE & Temp_Wait(LONG_5))
Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_3, Port_Name & " End Of File (EOF) Received       ", TEXT_EQUALS_SPACE & Temp_Wait(LONG_6))
Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_4, Port_Name & " Priority / Interrupt Character   ", TEXT_EQUALS_SPACE & Temp_Wait(LONG_7))
Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_5, Port_Name & " Input Queue (awaiting ReadFile)  ", TEXT_EQUALS_SPACE & Temp_Wait(LONG_8))
Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_5, Port_Name & " Output Queue (for Transmission)  ", TEXT_EQUALS_SPACE & Temp_Wait(LONG_9))

Else

Call PRINT_DEBUG_TEXT(Module_Name, TEXT_FAILURE, Port_Name & " Failed to Clear Com Errors")
...
Const TEXT_NEW_STATE  As String = " New Debug State = "
...
If Port_Valid Then
    ...
    End If
        ...
        Com_Port_String = TEXT_COM_PORT & CStr(Port_Number) & TEXT_COMMA & TEXT_NEW_STATE
...
Const TEXT_FRAME_TIME As String = ", Frame Duration="
Const TEXT_FRAME_LENGTH As String = ", Frame Length="
