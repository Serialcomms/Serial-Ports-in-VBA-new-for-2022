...
Private Declare PtrSafe Function Build_Com_DCB Lib "Kernel32.dll" Alias "BuildCommDCBA" (ByVal Config_Text As String, ByRef Port_DCB As DEVICE_CONTROL_BLOCK) As Boolean
...
(ByVal Port_Handle As LongPtr, ByRef Error_Mask As Long, ByRef Port_Status As COM_PORT_STATUS) As Boolean

Private Declare PtrSafe Function Com_Port_Open Lib "Kernel32.dll" Alias "CreateFileA" _
(ByVal Port_Name As String, ByVal PORT_ACCESS As Long, ByVal SHARE_MODE As Long, ByVal PORT_SECURITY_ATTRIBUTES As Any, _
 ByVal CREATE_DISPOSITION As Long, ByVal FLAGS_AND_ATTRIBUTES As Long, ByVal TEMPLATE_FILE_Handle As Any) As LongPtr
...
Dim Result_Text As String, Detail_Text As String, Temp_Port_Name As String
...
Temp_Result = False: Result_Text = TEXT_FAILURE
Temp_Port_Name = TEXT_COM_PORT & CStr(Port_Number) & TEXT_COMMA

If Port_Valid Then

Port_Handle = COM_PORT(Port_Number).Handle ' existing handle if port open
...
Temp_Handle = Com_Port_Open(Device_Path, GENERIC_RW, OPEN_EXCLUSIVE, ByVal vbNullString, OPEN_EXISTING, CREATE_FILE_FLAGS, ByVal vbNullString)
...
If GET_PORT_CONFIG(Port_Number) Then          ' get existing com port config (baud, parity etc.) into device control block
    ...
    If New_Settings Then
        ...
        Temp_Build = Build_Com_DCB(Port_Settings, COM_PORT(Port_Number).DCB)
...
If Port_Valid Then
    ...
    Call PRINT_SHOW_TEXT(Print_Text_1, Print_Text_6, Port_Name & Value_Text_01, COM_PORT(Port_Number).Settings)
    Call PRINT_SHOW_TEXT(Print_Text_1, Print_Text_2, Port_Name & Value_Text_02, COM_PORT(Port_Number).Timers.Frame_MilliSeconds)
    Call PRINT_SHOW_TEXT(Print_Text_1, Print_Text_2, Port_Name & Value_Text_03, COM_PORT(Port_Number).Timers.Frame_Microseconds)
    Call PRINT_SHOW_TEXT(Print_Text_1, Print_Text_3, Port_Name & Value_Text_04, COM_PORT(Port_Number).Timers.Bytes_Per_Second)
    Call PRINT_SHOW_TEXT(Print_Text_1, Print_Text_5, Port_Name & Value_Text_05, COM_PORT(Port_Number).Timers.Exit_Loop_Wait)
    Call PRINT_SHOW_TEXT(Print_Text_1, Print_Text_5, Port_Name & Value_Text_06, COM_PORT(Port_Number).Timers.Char_Loop_Wait)
    Call PRINT_SHOW_TEXT(Print_Text_1, Print_Text_5, Port_Name & Value_Text_07, COM_PORT(Port_Number).Timers.Data_Loop_Wait)
    Call PRINT_SHOW_TEXT(Print_Text_1, Print_Text_5, Port_Name & Value_Text_08, COM_PORT(Port_Number).Timers.Line_Loop_Wait)
    Call PRINT_SHOW_TEXT(Print_Text_1, Print_Text_5, Port_Name & Value_Text_09, COM_PORT(Port_Number).Timers.Read_Wait_Time)
    Call PRINT_SHOW_TEXT(Print_Text_1, Print_Text_5, Port_Name & Value_Text_10, COM_PORT(Port_Number).Timers.Timeslice_Bytes)
    Call PRINT_SHOW_TEXT(Print_Text_1, Print_Text_4, Port_Name & Value_Text_11, COM_PORT(Port_Number).Buffers.Read_Buffer_Length)
...
Const Purge_Text_2 As String = " Purge All Result = "
Const Purge_Text_3 As String = " Last DLL Error "
...
If Port_Debug Then
    Error_Text = Decode_System_Errors(COM_PORT(Port_Number).DLL_Error)
    Temp_String = COM_PORT(Port_Number).Name & Purge_Text_2 & Temp_Result & TEXT_COMMA & Purge_Text_3 & Error_Text
    Call PRINT_DEBUG_TEXT(Module_Name, Purge_Text_1, Temp_String)
End If
...
Const Print_Text_5 As String = " Last Error "
...
If Port_Valid Then
    ...
    If Port_Started(Port_Number) Then
        ...
        Error_Text = Decode_System_Errors(COM_PORT(Port_Number).DLL_Error)
        Call PRINT_DEBUG_TEXT(Module_Name, TEXT_ERROR, Print_Text_4 & Port_Name & Print_Text_5 & Error_Text)
...
Const Failed_Text As String = " Failed to Clear Com Errors "
...
Call PRINT_DEBUG_TEXT(Module_Name, TEXT_FAILURE, Port_Name & Failed_Text)
...
Case Else:                               Error_Text = ERROR_UNKNOWN & Error_Code
...
Dim VOLTAGE As String: VOLTAGE = "0.0000"
...
EXAMPLE = VOLTAGE                                       ' return read volts result back to function for subsequent use.
