...
Dim Device_Path As String, Error_Text As String, Result_Text As String, Detail_Text As String, Success_Text As String, Failure_Text As String
...
COM_PORT(Port_Number).Buffers.Timeslice_Bytes = Effective_Byte_Count
...
If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, "READ_BYTES", COM_PORT(Port_Number).Name & Temp_Text_2 & COM_PORT(Port_Number).Buffers.Timeslice_Bytes)
...
If Port_Valid Then
    ...
    Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_5, Port_Name & " Read/Write 1-Second Timeslice Bytes   = ", COM_PORT(Port_Number).Buffers.Timeslice_Bytes)
...
If Port_Valid Then
     ...
     If Port_Started(Port_Number) Then
        ...
        COM_PORT(Port_Number).Timers.Timing_QPC_Now = GET_HOST_MILLISECONDS

        Wait_Result = SYNCHRONOUS_WAIT_COM_PORT(Port_Number, Wait_MilliSeconds)
        
        COM_PORT(Port_Number).Timers.Timing_QPC_End = GET_HOST_MILLISECONDS

        Temp_Milliseconds = COM_PORT(Port_Number).Timers.Timing_QPC_End - COM_PORT(Port_Number).Timers.Timing_QPC_Now
...
If Port_Debug Then
    ...
    Call PRINT_DEBUG_TEXT(Module_Name, TEXT_STARTING, COM_PORT(Port_Number).Name & Temp_Text_02 & COM_PORT(Port_Number).Buffers.Timeslice_Bytes)
    COM_PORT(Port_Number).Timers.Timing_QPC_Now = GET_HOST_MICROSECONDS
...
End If
    ...
    Do
       If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, TEXT_READING, COM_PORT(Port_Number).Name & Temp_Text_04)
        ...
        Do
            Read_Return = SYNCHRONOUS_READ_COM_PORT(Port_Number, COM_PORT(Port_Number).Buffers.Timeslice_Bytes)
            ...
            If Not COM_PORT(Port_Number).Buffers.Read_Buffer_Empty Then
                ...
                Case Is = COM_PORT(Port_Number).Buffers.Timeslice_Bytes          ' assume more data available immediately, improve responsiveness.
        ...
        If Not COM_PORT(Port_Number).Buffers.Read_Timeout Then
     ...
     Loop Until COM_PORT(Port_Number).Buffers.Read_Timeout Or Not Read_Return
     
    If Port_Debug Then
    
        Receive_Microseconds = GET_HOST_MICROSECONDS - COM_PORT(Port_Number).Timers.Timing_QPC_Now
...
Call PRINT_DEBUG_TEXT(Module_Name, TEXT_TIMING, COM_PORT(Port_Number).Name & Temp_Text_2 & COM_PORT(Port_Number).Buffers.Timeslice_Bytes)

COM_PORT(Port_Number).Timers.Timing_QPC_Now = GET_HOST_MICROSECONDS

End If

For Loop_Counter = LONG_1 To Transmit_Length Step COM_PORT(Port_Number).Buffers.Timeslice_Bytes

    Byte_Pointer = (Loop_Counter + COM_PORT(Port_Number).Buffers.Timeslice_Bytes) - LONG_1
    Loop_Closing = IIf(Transmit_Length - Loop_Counter < COM_PORT(Port_Number).Buffers.Timeslice_Bytes, True, False)
    ...
    COM_PORT(Port_Number).Buffers.Write_Buffer = Mid$(Transmit_Text, Loop_Counter, COM_PORT(Port_Number).Buffers.Timeslice_Bytes)
...
If Port_Debug Then
    ...
    COM_PORT(Port_Number).Timers.Timing_QPC_End = GET_HOST_MICROSECONDS
    Transmit_Time = COM_PORT(Port_Number).Timers.Timing_QPC_End - COM_PORT(Port_Number).Timers.Timing_QPC_Now
...
If COM_PORT(Port_Number).DLL_Error = SYSTEM_ERRORS.SUCCESS Then
    ...
    If COM_PORT(Port_Number).Buffers.Synchronous_Bytes_Read = LONG_0 Then
        ...
        COM_PORT(Port_Number).Buffers.Read_Timeout = IIf(COM_PORT(Port_Number).Timers.Last_Data_Time > COM_PORT(Port_Number).Timers.Read_Wait_Time, True, False)
    ...
    Else
        ...
        COM_PORT(Port_Number).Timers.Last_Data_Time = LONG_0
        COM_PORT(Port_Number).Buffers.Read_Timeout = False
...
'-------------------------------------------------------------------------
Dim Port_Debug As Boolean: Const Module_Name As String = "GET_MODEM_LINES"
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'-------------------------------------------------------------------------

Dim Temp_Modem As Long
Dim Modem_Text As String, Error_Text As String
Dim Temp_Bool As Boolean
