...
Private Declare PtrSafe Function Send_Com_Escape Lib "Kernel32.dll" Alias "TransmitCommChar" (Port_Handle As LongPtr, ByVal Com_Char As Byte) As Long
...
Dim Config_Length As Long, Temp_Handle As LongPtr
...
Const CHARS_EXIT As Long = 2
Const CHARS_CHAR As Long = 5
Const CHARS_DATA As Long = 20
Const CHARS_LINE As Long = 100
...
COM_PORT(Port_Number).Timers.Exit_Loop_Wait = Int(LONG_1 + COM_PORT(Port_Number).Timers.Frame_MilliSeconds) * CHARS_EXIT
COM_PORT(Port_Number).Timers.Char_Loop_Wait = Int(LONG_1 + COM_PORT(Port_Number).Timers.Frame_MilliSeconds) * CHARS_CHAR
COM_PORT(Port_Number).Timers.Data_Loop_Wait = Int(LONG_1 + COM_PORT(Port_Number).Timers.Frame_MilliSeconds) * CHARS_DATA
COM_PORT(Port_Number).Timers.Line_Loop_Wait = Int(LONG_1 + COM_PORT(Port_Number).Timers.Frame_MilliSeconds) * CHARS_LINE
...
Dim Bytes_Per_Second As Long, Byte_Count As Long, Transmit_Length As Long, Timeslice_Bytes As Long
...
For Loop_Counter = LONG_1 To Transmit_Length Step Timeslice_Bytes
    ...
    If Port_Debug Then Temp_Text = Temp_Text_3 & Loop_Counter & " To " & Temp_Pointer & " (" & Byte_Count & " Bytes)"
...
Dim Temp_Bool As Boolean, Write_Complete As Boolean, Write_Buffer_Length As Long, Error_Text As String
...
Call PRINT_DEBUG_TEXT(Module_Name, "ERROR", COM_PORT(Port_Number).Name & Temp_Text_3 & Error_Text)
