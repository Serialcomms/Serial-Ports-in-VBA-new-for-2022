...
' Option Private Module  '  < Excel only - remove first comment mark to prevent function names from appearing in cell formula drop-down lists.
...
Private Type COM_PORT_PROFILE                              ' Not Microsoft - check/change locally if required
             ...
             Exit_Loop_Wait As Long                        ' Arbitrary loop wait time before read exit
...
Private Declare PtrSafe Function Kernel_Sleep_Milliseconds Lib "Kernel32.dll" Alias "Sleep" (ByVal Sleep_Milliseconds As LongPtr) As Long
...
Private Declare PtrSafe Function QPF Lib "Kernel32.dll" Alias "QueryPerformanceFrequency" (ByRef lpFrequency As Currency) As Long
Private Declare PtrSafe Function QPC Lib "Kernel32.dll" Alias "QueryPerformanceCounter" (ByRef lpPerfCounter As Currency) As Long
...
'--------------------------------------------------------------------------
Const Module_Name As String = "START_COM_PORT"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
...
'---------------------------------------------------------------------------
Const Module_Name As String = "COM_PORT_CREATE"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'---------------------------------------------------------------------------
...
Dim Device_Path As String, Result_Text As String, Detail_Text As String, Success_Text As String, Failure_Text As String
...
DLL_Error = Err.LastDllError: Error_Text = Decode_System_Errors(DLL_Error)
...
'---------------------------------------------------------------------
Const Module_Name As String = "COM_PORT_CONFIG"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'---------------------------------------------------------------------
...
'---------------------------------------------------------------------
Const Module_Name As String = "SET_PORT_CONFIG"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'---------------------------------------------------------------------
...
'
' The PORT_SETTINGS string must have the same structure as the equivalent DOS Mode command-line arguments for a COM Port:
' COMx[:][baud=b][parity=p][data=d][stop=s][to={on|off}][xon={on|off}][odsr={on|off}][octs={on|off}][dtr={on|off|hs}][rts={on|off|hs|tg}][idsr={on|off}]
' For example, to configure a baud rate of 1200, no parity, 8 data bits, and 1 stop bit, PORT_SETTINGS are :- "baud=1200 parity=N data=8 stop=1"

Dim Temp_Bool As Boolean
Dim Temp_String(LONG_1 To LONG_7) As String
Dim CONFIG_LENGTH As Long, Temp_Handle As Long
Dim Temp_Text As String, Result_Text As String, Detail_Text As String, Success_Text As String, Failure_Text As String
...
'---------------------------------------------------------------------
Const Module_Name As String = "SET_PORT_VALUES"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'---------------------------------------------------------------------
...
Dim Temp_Bool As Boolean
Dim Timeslice_Byte_Count As Boolean, Effective_Byte_Count As Long
...
If Len(COM_PORT(Port_Number).Read_Buffer) > LONG_0 Then

Temp_Bool = True

COM_PORT(Port_Number).Port_Data_Time = LONG_0
COM_PORT(Port_Number).Last_Data_Time = LONG_0
COM_PORT(Port_Number).Read_Wait_Time = LONG_125000

COM_PORT(Port_Number).Settings = GET_PORT_SETTINGS(Port_Number)
COM_PORT(Port_Number).Frame_MicroSeconds = GET_FRAME_TIME(Port_Number)
COM_PORT(Port_Number).Frame_MilliSeconds = COM_PORT(Port_Number).Frame_MicroSeconds / LONG_1000
COM_PORT(Port_Number).Bytes_Per_Second = Int(LONG_1 / COM_PORT(Port_Number).Frame_MicroSeconds * LONG_1E6)
COM_PORT(Port_Number).Read_Buffer_Length = Len(COM_PORT(Port_Number).Read_Buffer)

COM_PORT(Port_Number).Exit_Loop_Wait = Int(LONG_1 + COM_PORT(Port_Number).Frame_MilliSeconds) * LONG_2
COM_PORT(Port_Number).Char_Loop_Wait = Int(LONG_1 + COM_PORT(Port_Number).Frame_MilliSeconds) * LONG_5
COM_PORT(Port_Number).Data_Loop_Wait = Int(LONG_1 + COM_PORT(Port_Number).Frame_MilliSeconds) * LONG_20
COM_PORT(Port_Number).Line_Loop_Wait = Int(LONG_1 + COM_PORT(Port_Number).Frame_MilliSeconds) * LONG_100

If COM_PORT(Port_Number).Char_Loop_Wait > VBA_TIMEOUT / LONG_5 Then COM_PORT(Port_Number).Char_Loop_Wait = LONG_1000
If COM_PORT(Port_Number).Data_Loop_Wait > VBA_TIMEOUT / LONG_5 Then COM_PORT(Port_Number).Data_Loop_Wait = LONG_1000
If COM_PORT(Port_Number).Line_Loop_Wait > VBA_TIMEOUT / LONG_5 Then COM_PORT(Port_Number).Line_Loop_Wait = LONG_1000

Timeslice_Byte_Count = IIf(COM_PORT(Port_Number).Bytes_Per_Second < COM_PORT(Port_Number).Read_Buffer_Length, True, False)
Effective_Byte_Count = IIf(Timeslice_Byte_Count, COM_PORT(Port_Number).Bytes_Per_Second, COM_PORT(Port_Number).Read_Buffer_Length)

COM_PORT(Port_Number).Timeslice_Bytes = Effective_Byte_Count

Select Case COM_PORT(Port_Number).Bytes_Per_Second

    Case Is > LONG_1000: COM_PORT(Port_Number).Read_Wait_Time = LONG_100000
    Case Is < LONG_100:  COM_PORT(Port_Number).Read_Wait_Time = LONG_500000
    Case Else:           COM_PORT(Port_Number).Read_Wait_Time = LONG_125000

End Select

If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, "READ_BYTES", COM_PORT(Port_Number).Name & Temp_Text_2 & COM_PORT(Port_Number).Timeslice_Bytes)
If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, "BUFFER_SIZE", COM_PORT(Port_Number).Name & Temp_Text_3 & COM_PORT(Port_Number).Read_Buffer_Length)

Else   ' read buffer too small or not fixed size

Temp_Bool = False

If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, "BUFFER_SIZE", COM_PORT(Port_Number).Name & Temp_Text_1 & COM_PORT(Port_Number).Read_Buffer_Length)
...
If Port_Valid Then
    ...
    Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_2, Port_Name & " MilliSeconds per Read/Write character = ", COM_PORT(Port_Number).Frame_MilliSeconds)
    Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_2, Port_Name & " MicroSeconds per Read/Write character = ", COM_PORT(Port_Number).Frame_MicroSeconds)
    Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_6, Port_Name & " Standard Port Settings                = ", COM_PORT(Port_Number).Settings)
    Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_3, Port_Name & " Read/Write speed in Bytes per Second  = ", COM_PORT(Port_Number).Bytes_Per_Second)
    Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_5, Port_Name & " Exit Loop Wait Time Milliseconds      = ", COM_PORT(Port_Number).Exit_Loop_Wait)
    Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_5, Port_Name & " Char Loop Wait Time Milliseconds      = ", COM_PORT(Port_Number).Char_Loop_Wait)
    Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_5, Port_Name & " Data Loop Wait Time Milliseconds      = ", COM_PORT(Port_Number).Data_Loop_Wait)
    Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_5, Port_Name & " Line Loop Wait Time Milliseconds      = ", COM_PORT(Port_Number).Line_Loop_Wait)
    Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_5, Port_Name & " Synchronous Read Timeout Microseconds = ", COM_PORT(Port_Number).Read_Wait_Time)
    Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_5, Port_Name & " Read/Write 1-Second Timeslice Bytes   = ", COM_PORT(Port_Number).Timeslice_Bytes)
    Call PRINT_SHOW_TEXT(Temp_Text_1, Temp_Text_4, Port_Name & " Maximum Synchronous Read Buffer Size  = ", COM_PORT(Port_Number).Read_Buffer_Length)
...
'---------------------------------------------------------------------
Const Module_Name As String = "RELEASE_PORT"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'---------------------------------------------------------------------

' wrapper for com port release to provide debug progress information
...
Result_Text = TEXT_FAILURE: Detail_Text = Temp_String(LONG_3) & Error_Text
...
'---------------------------------------------------------------------
Const Module_Name As String = "STOP_COM_PORT"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'---------------------------------------------------------------------
...
Dim Result_Text As String, Detail_Text As String
Dim Temp_Bool As Boolean, Temp_Wait As Boolean, Port_Mode As Boolean
...
'---------------------------------------------------------------------
Const Module_Name As String = "WAIT_COM_PORT"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'---------------------------------------------------------------------
...
Dim Result_Text As String, Wait_Text As String
Dim Temp_Wait As Long, Temp_Milliseconds As Long
...
If Port_Valid Then
     ...
     If Port_Started(Port_Number) Then
        ...
        Call PRINT_DEBUG_TEXT(Module_Name, Temp_String_3, COM_PORT(Port_Number).Name & Temp_String_1 & Wait_MilliSeconds & TEXT_MS)
        
        COM_PORT(Port_Number).Timing_QPC_Now = GET_HOST_MILLISECONDS

        Wait_Result = SYNCHRONOUS_WAIT_COM_PORT(Port_Number, Wait_MilliSeconds)
        
        COM_PORT(Port_Number).Timing_QPC_End = GET_HOST_MILLISECONDS

        Temp_Milliseconds = COM_PORT(Port_Number).Timing_QPC_End - COM_PORT(Port_Number).Timing_QPC_Now

        Call PRINT_DEBUG_TEXT(Module_Name, Temp_String_4, COM_PORT(Port_Number).Name & Temp_String_5 & Temp_Milliseconds & Temp_String_2 & Wait_Result)
...
'---------------------------------------------------------------------
Const Module_Name As String = "SYNCHRONOUS_WAIT"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'---------------------------------------------------------------------
...
Loop_Iteration = Int(Wait_Remaining / Loop_Wait_Time)
...
Clear_Result = Clear_Com_Error(COM_PORT(Port_Number).Handle, COM_PORT(Port_Number).Line_Errors, COM_PORT(Port_Number).Status)
DLL_Error = Err.LastDllError: Error_Text = Decode_System_Errors(DLL_Error)
Clear_Success = IIf(DLL_Error = SYSTEM_ERRORS.SUCCESS, True, False)
...
If Clear_Success Then
    ...
    If Port_Debug Then Temp_Text = Temp_Text_5 & Queue_Length
    If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, TEXT_RESULT, COM_PORT(Port_Number).Name & Temp_Text)
...
'-------------------------------------------------------------------------
Const Module_Name As String = "RECEIVE_COM_PORT"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'--------------------------------------------------------------------------
...
Dim Receive_Microseconds As Currency, Receive_Timeout As Currency, Last_Read_Microseconds As Currency
...
COM_PORT(Port_Number).Receive_Result = vbNullString

If Port_Debug Then

    Call PRINT_DEBUG_TEXT(Module_Name, TEXT_SETTINGS, COM_PORT(Port_Number).Name & Temp_Text_01 & COM_PORT(Port_Number).Settings)
    Call PRINT_DEBUG_TEXT(Module_Name, TEXT_STARTING, COM_PORT(Port_Number).Name & Temp_Text_02 & COM_PORT(Port_Number).Timeslice_Bytes)
    COM_PORT(Port_Number).Timing_QPC_Now = GET_HOST_MICROSECONDS
...
End If
    ...
    Do
       If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, TEXT_READING, COM_PORT(Port_Number).Name & Temp_Text_04)
        ...
        Do
            Read_Return = SYNCHRONOUS_READ_COM_PORT(Port_Number, COM_PORT(Port_Number).Timeslice_Bytes)
            If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, TEXT_READING, COM_PORT(Port_Number).Name & Temp_Text_03 & Read_Return)
            
            If Read_Return Then
            
            If Not COM_PORT(Port_Number).Read_Buffer_Empty Then
            
                COM_PORT(Port_Number).Receive_Result = COM_PORT(Port_Number).Receive_Result & COM_PORT(Port_Number).Read_Result
                
                Select Case COM_PORT(Port_Number).Synchronous_Bytes_Read
                
                Case Is < LONG_4                                         ' assume manual data entry, improve responsiveness.
                If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, TEXT_SINGLE, COM_PORT(Port_Number).Name & Temp_Text_06 & COM_PORT(Port_Number).Char_Loop_Wait & TEXT_MS)
                Kernel_Sleep_Milliseconds COM_PORT(Port_Number).Char_Loop_Wait
            
                Case Is < LONG_21                                        ' assume continuous data ending, improve responsiveness.
                If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, TEXT_WAITING, COM_PORT(Port_Number).Name & Temp_Text_07 & COM_PORT(Port_Number).Data_Loop_Wait & TEXT_MS)
                Kernel_Sleep_Milliseconds COM_PORT(Port_Number).Data_Loop_Wait
                
                Case Is = COM_PORT(Port_Number).Timeslice_Bytes          ' assume more data available immediately, improve responsiveness.
                ...
                If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, TEXT_WAITING, COM_PORT(Port_Number).Name & Temp_Text_08 & COM_PORT(Port_Number).Line_Loop_Wait & TEXT_MS)
                Kernel_Sleep_Milliseconds COM_PORT(Port_Number).Line_Loop_Wait
        ...
        Loop Until COM_PORT(Port_Number).Read_Buffer_Empty Or Not Read_Return
        
        If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, TEXT_LOOPING, COM_PORT(Port_Number).Name & Temp_Text_11 & COM_PORT(Port_Number).Last_Data_Time)
        If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, TEXT_LOOPING, COM_PORT(Port_Number).Name & Temp_Text_12 & COM_PORT(Port_Number).Read_Wait_Time)
        
        If Not COM_PORT(Port_Number).Read_Timeout Then
        
        If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, "NO_DATA", COM_PORT(Port_Number).Name & Temp_Text_05 & COM_PORT(Port_Number).Exit_Loop_Wait & TEXT_MS)
        Kernel_Sleep_Milliseconds COM_PORT(Port_Number).Exit_Loop_Wait
        
        End If
                
     Loop Until COM_PORT(Port_Number).Read_Timeout Or Not Read_Return
    
    If Port_Debug Then
    
        Receive_Microseconds = GET_HOST_MICROSECONDS - COM_PORT(Port_Number).Timing_QPC_Now
        Receive_Byte_Count = Len(COM_PORT(Port_Number).Receive_Result)
...
RECEIVE_COM_PORT = COM_PORT(Port_Number).Receive_Result

End Function

Public Function TRANSMIT_COM_PORT(Port_Number As Long, Transmit_Text As String) As Boolean
'-------------------------------------------------------------------------
Const Module_Name As String = "TRANSMIT_COM_PORT"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'--------------------------------------------------------------------------
Dim Temp_Bool As Boolean, Loop_Closing As Boolean
Dim Baud_Rate As Long, Byte_Count As Long, Transmit_Length As Long
Dim Byte_Pointer As Long, Temp_Pointer As Long, Loop_Counter As Long
Dim Bytes_Per_Second As Long, Transmit_Time As Currency
...
Call PRINT_DEBUG_TEXT(Module_Name, TEXT_TIMING, COM_PORT(Port_Number).Name & Temp_Text_2 & COM_PORT(Port_Number).Timeslice_Bytes)

COM_PORT(Port_Number).Timing_QPC_Now = GET_HOST_MICROSECONDS

End If

For Loop_Counter = LONG_1 To Transmit_Length Step COM_PORT(Port_Number).Timeslice_Bytes

    Byte_Pointer = (Loop_Counter + COM_PORT(Port_Number).Timeslice_Bytes) - LONG_1
    Loop_Closing = IIf(Transmit_Length - Loop_Counter < COM_PORT(Port_Number).Timeslice_Bytes, True, False)
    ...
    COM_PORT(Port_Number).Write_Buffer = Mid$(Transmit_Text, Loop_Counter, COM_PORT(Port_Number).Timeslice_Bytes)
...
If Port_Debug Then
    ...
    COM_PORT(Port_Number).Timing_QPC_End = GET_HOST_MICROSECONDS
    Transmit_Time = COM_PORT(Port_Number).Timing_QPC_End - COM_PORT(Port_Number).Timing_QPC_Now
    Bytes_Per_Second = Transmit_Length / Transmit_Time * LONG_1E6
    Temp_String = Temp_Text_4 & Transmit_Length & Temp_Text_5 & Transmit_Time / LONG_1000 & TEXT_MS
...
'-------------------------------------------------------------------------
Const Module_Name As String = "GET_PORT_CONFIG"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'-------------------------------------------------------------------------
...
Dim Temp_Bool As Boolean
Dim Temp_Settings As String, Success_Text As String, Failure_Text As String, Progress_Text As String
...
'-------------------------------------------------------------------------
Const Module_Name As String = "GET_PORT_SETTINGS"
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
'-------------------------------------------------------------------------
...
'-------------------------------------------------------------------------
Const Module_Name As String = "SYNCHRONOUS_READ"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'-------------------------------------------------------------------------
...
Temp_Bool = Synchronous_Read(COM_PORT(Port_Number).Handle, COM_PORT(Port_Number).Read_Buffer, Read_Bytes_Requested, COM_PORT(Port_Number).Synchronous_Bytes_Read, ByVal vbNullString)
DLL_Error = Err.LastDllError

If DLL_Error = SYSTEM_ERRORS.SUCCESS Then

    If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, Temp_Text_1, COM_PORT(Port_Number).Name & Temp_Text_2 & COM_PORT(Port_Number).Synchronous_Bytes_Read)

    If COM_PORT(Port_Number).Synchronous_Bytes_Read = LONG_0 Then

        COM_PORT(Port_Number).Read_Buffer_Empty = True
        COM_PORT(Port_Number).Last_Data_Time = GET_HOST_MICROSECONDS - COM_PORT(Port_Number).Port_Data_Time
        COM_PORT(Port_Number).Read_Timeout = IIf(COM_PORT(Port_Number).Last_Data_Time > COM_PORT(Port_Number).Read_Wait_Time, True, False)
        COM_PORT(Port_Number).Read_Result = vbNullString
    
    Else
        
        COM_PORT(Port_Number).Read_Timeout = False
        COM_PORT(Port_Number).Read_Buffer_Empty = False
        COM_PORT(Port_Number).Last_Data_Time = LONG_0
        COM_PORT(Port_Number).Port_Data_Time = GET_HOST_MICROSECONDS
        COM_PORT(Port_Number).Read_Result = Left$(COM_PORT(Port_Number).Read_Buffer, COM_PORT(Port_Number).Synchronous_Bytes_Read)
...
Else
    ...
    Error_Text = Decode_System_Errors(DLL_Error)
    If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, Temp_Text_1, COM_PORT(Port_Number).Name & Temp_Text_3 & DLL_Error & TEXT_EQUALS & Error_Text)
...
'-------------------------------------------------------------------------
Const Module_Name As String = "SYNCHRONOUS_WRITE"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'-------------------------------------------------------------------------

Dim Temp_Bool As Boolean, Write_Buffer_Length As Long
...
Write_Buffer_Length = Len(COM_PORT(Port_Number).Write_Buffer)

Temp_Bool = Synchronous_Write(COM_PORT(Port_Number).Handle, COM_PORT(Port_Number).Write_Buffer, Write_Buffer_Length, COM_PORT(Port_Number).Synchronous_Bytes_Sent, ByVal vbNullString)
DLL_Error = Err.LastDllError
...
If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, Sync_Write, COM_PORT(Port_Number).Name & Temp_Text_3 & COM_PORT(Port_Number).Synchronous_Bytes_Sent)

Else

Temp_Bool = False
Error_Text = Decode_System_Errors(DLL_Error)
If Port_Debug Then Call PRINT_DEBUG_TEXT(Module_Name, Sync_Write, COM_PORT(Port_Number).Name & Temp_Text_1 & DLL_Error & " = " & Error_Text)
...
'-------------------------------------------------------------------------
Const Module_Name As String = "SEND_COM_PORT"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'-------------------------------------------------------------------------
...
Dim Temp_Bool As Boolean, Port_Handle As Long
...
'-------------------------------------------------------------------------
Const Module_Name As String = "PUT_COM_PORT"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'-------------------------------------------------------------------------
...
Dim Put_Com_Character As String
...
'-------------------------------------------------------------------------
Const Module_Name As String = "GET_COM_PORT"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'-------------------------------------------------------------------------
...
'-------------------------------------------------------------------------
Const Module_Name As String = "PURGE_COM_PORT"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'-------------------------------------------------------------------------
...
'-------------------------------------------------------------------------
Const Module_Name As String = "SET_PORT_TIMERS"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'-------------------------------------------------------------------------
...
Dim Temp_String(LONG_1 To LONG_3) As String
Dim Result_Text As String, Detail_Text As String, Success_Text As String, Failure_Text As String

Const Read_Timeout As Long = MAXDWORD
Const WRITE_CONSTANT As Long = 3000

Temp_String(LONG_1) = " Port Timers Not Set, Last Error = "
Temp_String(LONG_2) = " Port Timer Values Applied to Port,"
Temp_String(LONG_3) = " Read = " & Read_Timeout & "mS" & ", Write = " & WRITE_CONSTANT & "mS"
...
COM_PORT(Port_Number).Timeouts.Write_Total_Timeout_Constant = LONG_0           ' Add an additional 2 to 3 seconds delay before finally timing out. ?
...
Success_Text = Temp_String(LONG_2) & Temp_String(LONG_3)
Result_Text = IIf(Temp_Bool, TEXT_SUCCESS, TEXT_FAILURE)
Detail_Text = IIf(Temp_Bool, Success_Text, Failure_Text)
...
'-------------------------------------------------------------------------
Const Module_Name As String = "SHOW_PORT_TIMERS"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'-------------------------------------------------------------------------
...
'-------------------------------------------------------------------------
Const Module_Name As String = "SHOW_PORT_QUEUES"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'-------------------------------------------------------------------------
...
'-------------------------------------------------------------------------
Const Module_Name As String = "CHECK_COM_PORT"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'-------------------------------------------------------------------------

' Application.Volatile  ' - remove comment mark to allow this function to recalculate in Excel.
' https://docs.microsoft.com/en-us/office/vba/api/Excel.Application.Volatile

Dim Temp_Clear As Boolean, Temp_Queue As Long
...
If Port_Started(Port_Number) Then
        ...
        Temp_Clear = Clear_Com_Error(COM_PORT(Port_Number).Handle, COM_PORT(Port_Number).Line_Errors, COM_PORT(Port_Number).Status)
...
'-------------------------------------------------------------------------
Const Module_Name As String = "SHOW_PORT_ERRORS"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'-------------------------------------------------------------------------
...
Temp_Bool = Clear_Com_Error(COM_PORT(Port_Number).Handle, COM_PORT(Port_Number).Line_Errors, COM_PORT(Port_Number).Status)
DLL_Error = Err.LastDllError: Error_Text = Decode_System_Errors(DLL_Error)

Port_Error = COM_PORT(Port_Number).Line_Errors
Temp_Error(LONG_1) = IIf(Port_Error And Line_Errors.OVERFLOW, TEXT_TRUE, TEXT_FALSE)
Temp_Error(LONG_2) = IIf(Port_Error And Line_Errors.OVERRUN, TEXT_TRUE, TEXT_FALSE)
Temp_Error(LONG_3) = IIf(Port_Error And Line_Errors.PARITY, TEXT_TRUE, TEXT_FALSE)
Temp_Error(LONG_4) = IIf(Port_Error And Line_Errors.FRAME, TEXT_TRUE, TEXT_FALSE)
Temp_Error(LONG_5) = IIf(Port_Error And Line_Errors.BREAK, TEXT_TRUE, TEXT_FALSE)
...
'-------------------------------------------------------------------------
Const Module_Name As String = "SHOW_PORT_ALL"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'-------------------------------------------------------------------------
...
'-------------------------------------------------------------------------
Const Module_Name As String = "SHOW_PORT_MODEM"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'-------------------------------------------------------------------------
...
Temp_Bool = Get_Modem_Status(COM_PORT(Port_Number).Handle, PORT_SIGNALS)
DLL_Error = Err.LastDllError: Error_Text = Decode_System_Errors(DLL_Error)
...
'-------------------------------------------------------------------------
Const Module_Name As String = "CLEAR_COM_ERROR"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'-------------------------------------------------------------------------
...
Call PRINT_DEBUG_TEXT(Module_Text, "PORT_STOPPED", "COM Port " & Port_Number & ", Port Not Started")

End Sub

Private Sub PRINT_INVALID_TEXT(Module_Text As String, Port_Number As Long)

Const Port_Invalid_Text As String = " Invalid, Defined Port Range = "

Call PRINT_DEBUG_TEXT(Module_Text, "INVALID_PORT", "Port Number " & Port_Number & Port_Invalid_Text & COM_PORT_RANGE)
...
Case Line_Errors.BREAK: Temp_String = "BREAK"
Case Line_Errors.FRAME: Temp_String = "FRAME"
Case Line_Errors.OVERFLOW: Temp_String = "OVERFLOW"
Case Line_Errors.OVERRUN: Temp_String = "OVERRUN"
Case Line_Errors.PARITY: Temp_String = "PARITY"
...
'-------------------------------------------------------------------------
Const Module_Name As String = "SHOW_PORT_DCB"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'-------------------------------------------------------------------------
...
Call PRINT_SHOW_TEXT(Module_Name, "DEVICE CONTROL", COM_PORT(Port_Number).Name & DCB_TEXT(DCB_COUNTER), TEXT_EQUALS_SPACE & DCB_VALUE(DCB_COUNTER))
Next DCB_COUNTER
...
'-------------------------------------------------------------------------
Const Module_Name As String = "SHOW_PORT_STATUS"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'-------------------------------------------------------------------------
...
Private Function Extend_String(Input_String As Variant, Return_Length As Long) As String

Dim Input_Length As Long, Extend_Text As String
Dim Delta_Length As Long, Spaces_Length As Long

Dim Spaces_String As String * 100
...
Public Function GET_MODEM_LINES(Port_Number As Long) As String
'-------------------------------------------------------------------------
Const Module_Name As String = "GET_MODEM_LINES"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'-------------------------------------------------------------------------

Dim Temp_Modem As Long
Dim Modem_Text As String
Dim Temp_Bool As Boolean

Const Temp_Text_1 As String = " Modem     CTS = "
Const Temp_Text_2 As String = " Modem     DSR = "
Const Temp_Text_3 As String = " Modem     RING = "
Const Temp_Text_4 As String = " Modem     RLSD = "

If Port_Valid Then

If Port_Started(Port_Number) Then

Temp_Bool = Get_Modem_Status(COM_PORT(Port_Number).Handle, Temp_Modem)
DLL_Error = Err.LastDllError: Error_Text = Decode_System_Errors(DLL_Error)

If Temp_Bool Then

Modem_Text = vbNullString
Modem_Text = Modem_Text & TIMESTAMP & COM_PORT(Port_Number).Name & Temp_Text_1 & IIf(Temp_Modem And PORT_CONTROL.CTS_ON, TEXT_ON, TEXT_OFF) & vbCrLf
Modem_Text = Modem_Text & TIMESTAMP & COM_PORT(Port_Number).Name & Temp_Text_2 & IIf(Temp_Modem And PORT_CONTROL.DSR_ON, TEXT_ON, TEXT_OFF) & vbCrLf
'MODEM_TEXT = MODEM_TEXT & TIMESTAMP & COM_PORT(Port_Number).Name & Temp_Text_3 & IIf(Temp_MODEM And PORT_CONTROL.RING_ON, TEXT_ON, TEXT_OFF) & vbCrLf
'MODEM_TEXT = MODEM_TEXT & TIMESTAMP & COM_PORT(Port_Number).Name & Temp_Text_4 & IIf(Temp_MODEM And PORT_CONTROL.RLSD_ON, TEXT_ON, TEXT_OFF) & vbCrLf

Else

Modem_Text = TIMESTAMP & COM_PORT(Port_Number).Name & " Error retrieving Modem Status "

End If

Else

Modem_Text = "ERROR-STOPPED-PORT"

End If

Else

Modem_Text = "ERROR-INVALID-PORT"

End If

GET_MODEM_LINES = Modem_Text
...
'-------------------------------------------------------------------------
Const Module_Name As String = "GET_FRAME_TIME"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'-------------------------------------------------------------------------
...
'----------------------------------------------------------------------
Const Module_Name As String = "TEMPLATE"
Dim Port_Debug As Boolean, Error_Text As String, DLL_Error As Long
Dim Port_Valid As Boolean: Port_Valid = Port_Number_Valid(Port_Number)
If Port_Valid Then Port_Debug = COM_PORT(Port_Number).Debug
'----------------------------------------------------------------------
...
If START_COM_PORT(Port_Number) Then                               ' continue if port starts correctly.
    ...
    Kernel_Sleep_Milliseconds 200                                 ' allow remote device to stabilise after opening port (optional)
...
Kernel_Sleep_Milliseconds 200                                    ' allow remote device to stabilise before closing port (optional)
